...
def main():
  ...
  for line in f_in.readlines():
    # tokenize line to [domain, party, frequency]
    e = re.compile('([^:]+):([^\t]+)\t([0-9]+)$');
    ...
    if (not m):
      ...
      party  = m.group(2);
      frequency = m.group(3);
      if (party == "first_domain"):
        input_first_dict[domain] = frequency;
      elif (party == "third_domain"):
  ...
  for line in f_in.readlines():
    ...
    if domain in lookup_dict:
      print "[HIT]", domain, party, frequency, rank
    else:
      print "[MISS]", domain, party, frequency, rank
  f_in.close();
